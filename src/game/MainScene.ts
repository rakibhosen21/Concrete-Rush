...
export default class MainScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.time.addEvent({ delay: 600, callback: this.spawnItem, callbackScope: this, loop: true });
    this.time.addEvent({ delay: 800, callback: this.spawnTree, callbackScope: this, loop: true });
  ...
  private spawnItem() {
    ...
    // Spawn 2 coins at once in different lanes
    const lanes = [0, 1, 2];
    ...
    for (let i = 0; i < 2; i++) {
        ...
        // If second spawn is a boost, maybe skip it to keep boosts rare
        if (i > 0 && type === 'boost-tex') {
            item.destroy();
        }
    }
  }
  ...
  private spawnTree() {
    ...
    const tree = this.treeGroup.get(0, 0, 'tree-tex');
    if (tree) {
        tree.setActive(true).setVisible(true);
        tree.setData('z', 0.01);
        tree.setData('side', side);
        // Random offset from road edge
        tree.setData('offset', 0.6 + Math.random() * 0.4);
    }
  }
  ...
  private drawPseudo3DRoad() {
    ...
    this.roadWidthBottom = width * 0.85;
    const currentRoadWidthBottom = this.roadWidthBottom;
  ...
  private updateObjectsPerspective(delta: number) {
    ...
    const roadWidth = this.roadWidthBottom;
    ...
    [...this.obstacles.getChildren(), ...this.items.getChildren()].forEach((obj: any) => {
        ...
        const screenX = centerX + ((obj.getData('lane') - 1) * (roadWidth * 0.33 * z));
        
        obj.x = screenX;
        obj.y = targetY;
        
        // Coins scale z*3, others z*2
        const isCoin = obj.texture.key === 'coin-tex';
        obj.setScale(isCoin ? Math.max(z * 3, 0.5) : z * 2);
        
        // Coins at 40, Obstacles at 45 (just below car at 50)
        const baseDepth = isCoin ? 40 : 45;
    ...
    // Update Trees
    this.treeGroup.getChildren().forEach((tree: any) => {
        let z = tree.getData('z') || 0;
        z += zStep;
        tree.setData('z', z);

        const targetY = this.horizonY + (height - this.horizonY) * z;
        const sideMult = tree.getData('side') === 'left' ? -1 : 1;
        const screenX = centerX + (sideMult * (roadWidth * 0.5 * z)) + (sideMult * tree.getData('offset') * 200 * z);
  ...
  update(time: number, delta: number) {
    ...
    // Player and Parallax
    const roadWidth = this.roadWidthBottom;
    const targetX = (this.scale.width / 2) + ((this.currentLane - 1) * (roadWidth * 0.33 * 0.85)); // 0.85 is player Z approx
